-- =============================================================================
-- FOCT BuildingOps — ROSTER APPLY (paste into the Supabase SQL editor and Run).
-- Idempotent — safe to re-run.
--
-- Makes the roster honest: a shift must finish after it starts, the same person
-- cannot be rostered twice at once, one write path that explains its refusals,
-- and a week-copy that reports what it skipped instead of dropping it quietly.
-- The board and the timesheet's "rostered" figure read the same rows.
--
-- Only needed if you applied APPLY_EVERYTHING.sql before this existed.
-- Afterwards run supabase/tests/roster_isolation_check.sql (expect 14 ok).
-- =============================================================================
