-- =============================================================================
-- FOCT BuildingOps — GETTING IN (paste into the Supabase SQL editor and Run).
-- Idempotent — safe to re-run.
--
-- Stops sign-in being a manual job. After this:
--
--   * a new account gets its profile row by itself;
--   * an admin invites a colleague by EMAIL from Settings → People, and that
--     person is in the moment they sign in — no dashboard, no SQL;
--   * on a project nobody has ever signed in to, THE FIRST PERSON TO SIGN IN
--     becomes an admin of the organisation that runs the site. After that the
--     door is shut: an uninvited arrival gets nothing.
--
-- Afterwards run supabase/tests/auth_onboarding_check.sql (expect 15 ok).
--
-- ONE DASHBOARD SETTING STILL MATTERS, and it is worth two minutes:
--   Authentication → URL Configuration → Site URL = your app's URL
--   (add https://<your-app>/auth/callback under Redirect URLs too).
-- Without it, the emailed link points at localhost and will not sign anyone in.
-- =============================================================================
