-- =============================================================================
-- FOCT BuildingOps — KIOSK SIGN-IN v1 APPLY (paste into the Supabase SQL editor
-- and Run). Idempotent — safe to re-run.
--
-- Adds NOTICES (general + personal, multi-language, acknowledgeable) and
-- OFFLINE SYNC for the kiosk tablet: bootstrap/cache, an outbox that upserts on
-- a client-generated id so a replay can never double-punch, and per-employee
-- PIN hashes for verifying a PIN with no network.
--
-- Only needed if you already applied APPLY_EVERYTHING.sql BEFORE this existed —
-- a fresh APPLY_EVERYTHING now contains it.
--
-- Afterwards run supabase/tests/notices_isolation_check.sql (expect 18 ok
