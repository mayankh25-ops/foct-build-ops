-- =============================================================================
-- FOCT BuildingOps — STAGE 2 PHASE 2 APPLY (paste into the Supabase SQL editor
-- and Run). Idempotent — safe to re-run.
--
-- Adds ATTENDANCE + KIOSK DEVICES: cleaners with system-generated 4-digit PINs,
-- paired kiosk tablets (the Android APK / cleaners-room iPad), sign-in/out
-- events with selfies, and the paired-session view the timesheet screen reads.
--
-- Only needed if you already applied APPLY_EVERYTHING.sql BEFORE this was added
-- to it — a fresh APPLY_EVERYTHING now contains all of this already.
--
-- Afterwards run supabase/tests/kiosk_isolation_check.sql (expect 19 ok
-- notices). It creates two test cleaners and a test tablet on the demo
-- building; delete them from Staff afterwards if you don't want them.
-- =============================================================================
