-- =============================================================================
-- APPLY_SITES.sql — adds sites from inside the app, and tells you what's
-- missing when something doesn't load.
--
-- WHAT THIS FIXES
--   * "I can't add a new site."  There is a New site button now (Settings →
--     Sites). This creates the RPC behind it.
--   * "Cleaners / kiosk tablets / notices / attendance just spin forever."
--     They were waiting on a building you had no way to create.
--   * "It says my database is missing something but not what."  Settings →
--     System health now names it, and names the file that fixes it.
--
-- HOW TO USE
--   1. Supabase dashboard → SQL Editor → paste this whole file → Run.
--   2. Reload the app and open Settings → System health. It should be green.
--   3. Settings → Sites → New site.
--
-- Safe to run more than once. If you would rather paste one file than three,
-- APPLY_EVERYTHING.sql contains this and everything before it.
--
-- Verify with: supabase/tests/site_create_check.sql (17 ok)
--              supabase/tests/health_check.sql      (9 ok — DESTRUCTIVE, only
--                                                    ever on a scratch database)
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 0017_site_create.sql
-- ---------------------------------------------------------------------------
-- =============================================================================
