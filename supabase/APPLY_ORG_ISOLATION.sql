-- =============================================================================
-- FOCT BuildingOps — ORG ISOLATION + TODAY (paste into the Supabase SQL editor
-- and Run). Idempotent — safe to re-run.
--
-- APPLY THIS ONE PROMPTLY. It closes a real hole: before it, every organisation
-- attached to a building — the concierge company, the strata manager, an
-- electrical subcontractor, a competing cleaning company — could read the
-- cleaners' staff rows through the API, INCLUDING the plaintext 4-digit kiosk
-- PIN, plus their check-ins, roster and timesheets. A PIN is a credential, so
-- that was enough to sign somebody else in.
--
-- After this, staff records belong to the organisation that EMPLOYS them, the
-- PIN and device-token columns cannot be selected by any API role at all, and
-- a tablet only ever sees its own company's people.
--
-- It also adds `attendance_day()` — today's board (on site / late / no check-in
-- / finished), which is what the building owner and concierge get instead of
-- names: counts, so cleaning progress is still visible.
--
-- Then run, in this order:
--   supabase/tests/org_isolation_check.sql          → expect 22 ok notices
--   supabase/tests/attendance_day_isolation_check.sql → expect 16 ok notices
--
-- NOTE: after this, a new column added to `staff` or `kiosk_devices` is not
-- readable by the app until it is added to the column grants in 0012.
-- =============================================================================
