...
## 2026-07-21 — Concierge amenity booking + availability calendar (owner directive)
- **Book amenity** (button on the Amenity bookings section, lg modal): amenity (with location + capacity), resident it's for, date, start + duration, guests, note. **Desk bookings confirm immediately** — the concierge is front-of-house; residents' own requests keep going through pending → manager confirmation.
- **Live availability check:** `conflictingBookings` (store helper — overlap on amenity × date × time window, ignoring declined) drives the modal: a clash renders a red "Not available — already booked 19:00–22:00 · Nguyen" panel and DISABLES confirm; a free slot renders a green "Available — nothing clashes" panel. Over-capacity guests get a warning but don't block (resident's call).
- **Availability calendar:** the section gained a Requests / Availability calendar toggle. The calendar is a Mon–Sun week grid — one row per amenity (location + capacity), today's column highlighted, bookings as green (confirmed) / amber (pending) time chips with the resident's surname, EMPTY CELL = AVAILABLE ALL DAY (stated in the header). Prev/next week + This week navigation; horizontal scroll on narrow screens.
Verified: 6/6 new checks (seeded clash blocks confirm, free slot confirms instantly, grid renders all six amenities, the new booking's chip appears, week navigation) + regression 17/17 + 9/9 + 23/23 · tokens ✓ · tsc ✓ · build ✓.

## 2026-07-21 — STAGE 2 OPENED (owner approved the 8-phase plan) · Phase 1 shipped: logins & roles
Plan approved verbatim: 1 logins/roles → 2 attendance backend → 3 Vercel deploy → 4 service desk → 5 calendar → 6 consumables → 7 residents → 8 security + CI. Same delivery method as Stage 4: prove each migration on the local PG16 mirror, ship a paste-ready APPLY bundle, swap store data sources without touching screens, merge only on green.
**Phase 1:**
- `0006_session_profile.sql` — `current_profile()` SECURITY DEFINER RPC: the caller's users row, ACTIVE memberships (org id/name/slug/type + role key/name) and the buildings their orgs service; null when unauthenticated; EXECUTE granted to `authenticated` only. Proven on the mirror (idempotent double-apply) with `tests/session_profile_check.sql` — 5 ok notices (anon null, caller identity, org+role exposure, NO foreign memberships, buildings scoped to own orgs). Paste bundle: `APPLY_STAGE2_PHASE1.sql`.
- `src/lib/session.ts` — session store with four states: **demo** (no Supabase env — sandbox/fresh clones keep full demo behaviour, nothing redirects), loading, signed-out, ready; `onAuthStateChange` follows sign-in/out/refresh; `activeOrgIdOr(fallback)` gives call sites the session org in live mode.
- AppShell mounts the session; **live + signed-out → redirect /sign-in**; TopBar shows the REAL name/role/org/building from the profile (demo persona as fallback) and a Sign out button in live mode. Integrations store now acts as the session org instead of the pinned DEMO_ORG_ID.
- First-membership = active org for now; the org switcher lands when a multi-org user exists.
Verified: mirror 5/5 · demo-mode e2e (no redirect, no sign-out button, demo persona) · smoke 9/9 + 17/17 · tokens ✓ · tsc ✓ · build ✓. **Owner action: paste `APPLY_STAGE2_PHASE1.sql`, run `tests/session_profile_check.sql` (expect 5 ok), then sign in on your machine — your real name/org appear in the top bar.**

## 2026-07-24 — Sign-in diagnosability (owner blocked 3 days on "Invalid API key")
Root-cause analysis rather than another guess: **"Invalid API key" and "Invalid login credentials" are different failures.** The first is rejected at the door — the password is never evaluated — and is virtually always a URL/key pair from TWO DIFFERENT projects (the owner has a stale second project his dashboard kept drifting to; the key was copied from its settings page while the URL pointed at the real one). The second means the key is fine and only the credential is wrong. Nothing in the app was broken; the app just parroted the raw string and gave the owner no way to tell these apart.
- **`npm run check:supabase`** (scripts/check-supabase.mjs) — a connection doctor that verdicts in plain English: reads .env.local, flags paste injuries (placeholder text, `<>` brackets, embedded spaces, a `sb_secret_` key in a public var), extracts the project ref from the URL, DECODES legacy anon JWTs to compare their embedded `ref` (catches cross-project keys **offline, instantly**), then probes /auth/v1/settings and finally an optional real sign-in (`npm run check:supabase -- email password`). Each verdict prints the exact dashboard URL for the project it's actually configured for.
- **Strictness fix found in self-testing:** the first cut treated a network-proxy 403 as success — a false all-clear, precisely the failure mode that wasted the owner's time. A reply now only counts as Supabase's if it is HTTP 200 with JSON; proxy/egress blocks, paused projects and WAF pages each get their own message.
- **Sign-in page** now maps errors to actionable text ("Configuration problem, not your password: this app's key was rejected by project X… run npm run check:supabase") instead of the raw string.
Verified: doctor exercised against missing file, placeholder, bracketed key, cross-project JWT (correctly named the foreign project), and a blocked network — right verdict in every case; tokens ✓ tsc ✓ build ✓.

## 2026-07-25 — The two-project split, fully diagnosed: APPLY_EVERYTHING + project inventory
The owner's login saga had a SECOND root cause hiding behind the first. Once the connection doctor proved the key was right and `testcctv59` signed in, the org-linking SQL failed at `public.users` — but only AFTER finding the auth user. That ordering is the proof: **`iitdhglgatyoxjocxprs` (the project the app points at, where the working login lives) had auth users but NO platform tables** — every APPLY bundle had been pasted into the OTHER project. So sign-in worked while every data query would have failed. The two projects each held half of a working system.
- **`supabase/APPLY_EVERYTHING.sql`** (`npm run build:apply-everything`, generator in scripts/) — one paste that takes an EMPTY project to fully set up: 0000 foundation → 0001 theme engine → seed → 0002 service desk → 0003 anon hardening → seed_service_desk → 0004 authz → 0005 integrations → 0006 session profile. **Proven on a fresh PG16 database: applies cleanly, re-runs idempotently, and session_profile_check passes 5/5 afterwards** (tables present, 5 orgs seeded).
- **`supabase/tests/project_inventory.sql`** — one query, safe in any project (uses `to_regclass`, never errors on missing tables), answering "which project is this tab on and what does it contain": auth user count + emails, and booleans for platform tables / organisations / service desk / integrations / session profile. Verified against both a set-up database and a bare one.
- Sign-in now redirects to /dashboard on success (it previously left the user parked on the sign-in page with only a message).
Guidance recorded for the owner: consolidate on `iitdhglgatyoxjocxprs` — paste APPLY_EVERYTHING there, re-link the account, and delete the stale project. The inventory query is the permanent answer to "am I in the right project?".

## 2026-07-25 — Half-built project repair: RESET_PUBLIC_SCHEMA (owner hit `column b.owner_org_id does not exist`)
APPLY_EVERYTHING failed on the owner's project even though it applies cleanly to an empty database. Cause reproduced deterministically on the mirror: the project held an OLDER partial `public.buildings` (no `owner_org_id`) from an interrupted early apply, and **`create table if not exists` skips an existing table rather than upgrading it** — so the schema stayed stuck at the old shape while later objects referenced the new columns. This is the standing hazard of if-not-exists idempotency, and it will bite any half-built project.
- **`supabase/RESET_PUBLIC_SCHEMA.sql`** — drops + recreates the `public` schema (and `app`), restoring the grants Supabase expects, so APPLY_EVERYTHING can rebuild from a known-good state. **Logins are preserved** — accounts live in the separate `auth` schema, untouched; storage untouched. Clearly labelled as destructive to app data and intended for demo/dev projects.
- Verified end-to-end on the mirror: seeded a drifted database → APPLY_EVERYTHING FAILS with the owner's exact error → RESET + APPLY → succeeds, tables present, and the pre-existing auth user survives the rebuild.
- supabase/README.md gains a situation → file table (new project / unknown project / half-built project) plus the note that seeded demo `auth.users` rows appear in the dashboard but cannot sign in.
